...
def _device_dict(d: Device) -> dict:
    return {
        ...
        "last_cpu_temp_c": d.last_cpu_temp_c,
...
# ── Agent one-liner installer ─────────────────────────────────────────────────

@router.get("/agent/bootstrap", include_in_schema=False)
async def agent_bootstrap(request: Request):
    """
    Returns a shell script that downloads and installs the agent in one command.
    No authentication required — the enrollment secret gates actual device registration.
...
#
# Usage:
#   curl -fsSL {server_url}/v1/agent/bootstrap | sudo bash -s -- --secret <SECRET>
#
set -euo pipefail
...
async def agent_package():
    """
    Serves the agent source tree as a tar.gz archive.
    Used by the bootstrap script — no authentication required.
    """
    import io
    import os
    import tarfile as tf

    bundle_path = "/agent-bundle"
    if not os.path.isfile(os.path.join(bundle_path, "install.sh")):
        raise HTTPException(
            status_code=503,
            detail=(
